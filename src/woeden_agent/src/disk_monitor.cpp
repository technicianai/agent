...
namespace woeden
{
disk_monitor::disk_monitor(shared_ptr<mqtt_facade> facade)
: Node("woeden_disk_monitor"), facade_(facade)
{
  sample();
...
vector<mount> disk_monitor::get_mounts() { return mounts_; }
...
void disk_monitor::sample()
{
  ...
  while (getline(ss, line)) {
    ...
    if (access(path.c_str(), W_OK) == 0 && !S_ISREG(path_stat.st_mode)) {
      mounts.push_back({.path = path, .available = available, .total = total});
    }
  }
