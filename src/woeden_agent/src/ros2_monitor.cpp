...
namespace woeden
{
ros2_monitor::ros2_monitor(
  shared_ptr<mqtt_facade> facade, shared_ptr<recording_manager> rm,
  vector<recording_trigger> triggers)
: Node("woeden_ros2_monitor"), rm_(rm), facade_(facade), unassigned_triggers_(triggers)
{
  discover_packages();
  ...
  service_ = this->create_service<interfaces::srv::Record>(
    "record", bind(&ros2_monitor::custom_trigger_fired, this, _1, _2));
  publisher_ = this->create_publisher<interfaces::msg::WrappedBytes>("/woeden", 10);
}

void ros2_monitor::add_trigger(recording_trigger rt) { unassigned_triggers_.push_back(rt); }
...
void ros2_monitor::discover_packages()
{
  ...
  while (getline(ss, line)) {
    ...
    if (!known_pkg) {
      rc.packages.push_back({.name = pkg_name, .executables = {exec}});
    }
  }
...
void ros2_monitor::discover_nodes()
{
  ...
  for (string & node_name : node_names) {
    ...
    if (
      new_node && node_name != "/ros_bridge" && node_name != "/woeden_disk_monitor" &&
      node_name != "/woeden_ros2_monitor" && node_name != "/woeden_trigger_worker" &&
      node_name != "/woeden_upload_worker") {
...
void ros2_monitor::discover_topics()
{
  ...
  for (auto it = topic_names_and_types.begin(); it != topic_names_and_types.end(); ++it) {
    ...
    try {
      topic * t = new topic{
        .name = topic_name, .type = topic_type, .message_count = 0, .triggers = new_topic_triggers};
      topics_.push_back(t);

      rclcpp::SubscriptionBase::SharedPtr sub = nullptr;
      if (topic_type == "std_msgs/msg/String") {
        function<void(shared_ptr<std_msgs::msg::String>)> cb =
          bind(&ros2_monitor::json_trigger_callback, this, _1, t);
        sub = create_subscription<std_msgs::msg::String>(topic_name, 10, cb);
      } else if (topic_type == "diagnostic_msgs/msg/KeyValue") {
        function<void(shared_ptr<diagnostic_msgs::msg::KeyValue>)> cb =
          bind(&ros2_monitor::key_value_trigger_callback, this, _1, t);
        ...
        function<void(shared_ptr<diagnostic_msgs::msg::DiagnosticStatus>)> cb =
          bind(&ros2_monitor::status_trigger_callback, this, _1, t);
        ...
        function<void(shared_ptr<diagnostic_msgs::msg::DiagnosticArray>)> cb =
          bind(&ros2_monitor::status_array_trigger_callback, this, _1, t);
        sub = create_subscription<diagnostic_msgs::msg::DiagnosticArray>(topic_name, 10, cb);
      } else {
        function<void(shared_ptr<rclcpp::SerializedMessage>)> cb =
          bind(&ros2_monitor::default_callback, this, _1, t);
...
void ros2_monitor::default_callback(shared_ptr<rclcpp::SerializedMessage> msg, topic * t)
{
  ...
  vector<unsigned char> contents(
    serialized_message.buffer, serialized_message.buffer + serialized_message.buffer_length);
...
void ros2_monitor::key_value_trigger_callback(
  shared_ptr<diagnostic_msgs::msg::KeyValue> msg, topic * t)
{
  t->message_count++;
  for (recording_trigger rt : t->triggers) {
...
void ros2_monitor::status_trigger_callback(
  shared_ptr<diagnostic_msgs::msg::DiagnosticStatus> msg, topic * t)
{
  t->message_count++;
  for (recording_trigger rt : t->triggers) {
    if (rt.is_enabled() && rt.evaluate(msg)) {
...
void ros2_monitor::status_array_trigger_callback(
  shared_ptr<diagnostic_msgs::msg::DiagnosticArray> msg, topic * t)
{
  t->message_count++;
  for (recording_trigger rt : t->triggers) {
    if (rt.is_enabled() && rt.evaluate(msg)) {
...
void ros2_monitor::gateway_cmd(string ec2_ip)
{
  ...
  char * args[10] = {
    ...
    "-R",
    ":9090:localhost:9090",
    const_cast<char *>(ec2_login.c_str()),
    NULL};
  execvp("ssh", args);
}
...
void ros2_monitor::custom_trigger_fired(
