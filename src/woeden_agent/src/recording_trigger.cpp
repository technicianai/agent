...
recording_trigger::recording_trigger(
  uint32_t id, string topic, string type, bool enabled, vector<recording_topic> record_topics,
  uint32_t duration, string base_path, string msgdef)
{
  id_ = id;
  topic_ = topic;
  type_ = type;
...
uint32_t recording_trigger::get_id() { return id_; }

string recording_trigger::get_topic() { return topic_; }

string recording_trigger::get_topic_type() { return type_; }

vector<recording_topic> recording_trigger::get_record_topics() { return record_topics_; }

uint32_t recording_trigger::get_duration() { return duration_; }

string recording_trigger::get_base_path() { return base_path_; }

bool recording_trigger::is_enabled() { return enabled_; }
...
void recording_trigger::set_status_comparison(status_comparison * sc) { status_comparison_ = sc; }
...
void recording_trigger::set_enabled(bool enabled) { enabled_ = enabled; }
...
recording_trigger recording_trigger::from_json(nlohmann::json rt_json)
{
  ...
  recording_trigger rt = recording_trigger(
    rt_json["id"].get<uint64_t>(), rt_json["topic"].get<string>(), type,
    rt_json["enabled"].get<bool>(), record_topics, rt_json["duration"].get<uint32_t>(),
    rt_json["base_path"].get<string>(), msgdef);
